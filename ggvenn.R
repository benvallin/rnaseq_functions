...
ggvenn_ben <- function(data, columns = NULL, show_elements = FALSE, show_percentage = TRUE, 
                       digits = 1, fill_color = viridis::viridis(length(data)), 
                       fill_alpha = 0.5, stroke_color = "black", stroke_alpha = 1, 
                       stroke_size = 1, stroke_linetype = "solid", set_name_color = viridis::viridis(length(data)), 
                       set_name_size = 2, text_color = "black", text_size = 2, label_sep = ",", 
                       count_column = NULL, show_outside = c("auto", "none", "always"), 
                       auto_scale = FALSE) 
{
  show_outside <- match.arg(show_outside)
  venn <- prepare_venn_data(data, columns, show_elements, show_percentage, 
                            digits, label_sep, count_column, show_outside, auto_scale)
  g <- venn$shapes %>% mutate(group = LETTERS[group]) %>% ggplot() + 
    geom_polygon(aes(x = x, y = y, group = group, fill = group), 
                 alpha = fill_alpha) + geom_polygon(aes(x = x, y = y, 
                                                        group = group), fill = NA, color = stroke_color, size = stroke_size, 
                                                    alpha = stroke_alpha, linetype = stroke_linetype)
  if (nrow(venn$labels) > 0) {
    ...
    #                    size = set_name_size)
    # NEW:
    g <- g + ggrepel::geom_text_repel(data = venn$labels,
                                      aes(x = x,
                                          y = y,
                                          label = text),
                                      color = set_name_color,
